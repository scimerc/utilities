		...
		unless ( $finfo[$personcoord] eq "0" ) # person id is available
		{
			...
			while ( my $pline = <$ph> ) # scan person file to see if person is there
			{
				...
				if ( $pline =~ /^${finfo[$personcoord]}[\t ]*/ ) # person id matches that in person file
				{
					...
					print "0\t0\t${sex}\n";
					$oinfo[$count] = 0; # no orphan, or at least in family file
					$found = 1; # set flag
				}
				$count++;
			}
			...
			if ( $found == 0 ) # the person was not found in person file
			{
				...
				while ( my $ffline = <$ffh> ) # scan family file to see if the person is a parent
				{
					...
					$sex = 1 if ( $ffinfo[$fathercoord] eq $finfo[$personcoord] );
					$sex = 2 if ( $ffinfo[$mothercoord] eq $finfo[$personcoord] );
...
# 					}
	...
	foreach my $oline ( @plist )
	{
		if ( $oinfo[$count] == 1 )
		{
			...
			# write down known information
			print "${pinfo[$numcoord]}\t0\t0\t0\t0\t${sex}\n";
		}
		$count++;
	}
	close $fh;
...
{
	print ( "\n generates a genealogy file out of a family relationship file and a person file.\n" );
	print ( "\n  usage: $0 <family file> <person file>\n" );
	print ( "\n  [note: make sure your family file is complete and no headers are present in any files.]\n\n" );
}
